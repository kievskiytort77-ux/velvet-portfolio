...
async def receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    headers = {
        ...
        "x-upsert": "true"  # перезапись если файл уже есть
    }
    ...
    if r.status_code in [200, 201]:
        photo_url = f"{SUPABASE_URL}/storage/v1/object/public/products/{filename}"
        ...
        logger.error(f"Supabase storage error: {r.status_code} {r.text}")
        await update.message.reply_text("❌ Ошибка загрузки фото. Попробуй ещё раз.")
        return WAIT_PHOTO
...
async def save_product(message, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        old_text = f" (було {product['old_price']}₴)" if product.get("old_price") else ""
        size_text = f"\n📏 Розміри: {product['size']}" if product.get("size") else ""
